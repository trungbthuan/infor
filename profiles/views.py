...
def profile_ModelForm_create(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Thay bằng tên URL list view của bạn
            return redirect('profiles_list')
...
def profile_create(request):
    if request.method == 'POST':
        try:
            # Tạo dữ liệu trực tiếp vào Database
            ...
            # Chuyển hướng về trang danh sách (Sửa tên 'profiles_list' cho đúng urls của bạn)
            return redirect('profiles_list')

        except Exception as e:
            # Nếu có lỗi (Ví dụ: sai định dạng ngày tháng)
            context = {
                'errors': {'detail': str(e)},
                'form_data': request.POST,  # Giữ lại dữ liệu cũ để user không phải nhập lại
            }
    ...
    #         return redirect('profiles_call_profile_create')
    #     else:
    ...
    #             error_details = {'non_field_errors': [
...
def profiles_delete_by_id(request, id):
    try:
        # Tìm và xóa trực tiếp trong DB
        student = Profiles.objects.get(id=id)
        student.delete()
        return redirect('update')
    except Profiles.DoesNotExist:
        return render(request, 'notification.html', {'message': f'Sinh viên ID {id} không tồn tại.'})
    except Exception as e:
        return render(request, 'notification.html', {'message': f'Lỗi: {str(e)}'})
...
def profiles_search_name(request):
    # 1. Lấy giá trị từ ô nhập liệu (input name="search_fields")
    search_query = request.GET.get('search_fields', '').strip()

    try:
        # 2. Truy vấn trực tiếp bằng Model với Q object (Tìm kiếm LIKE trong SQL)
        ...
        if search_query:
            profiles = Profiles.objects.filter(
                ...
                Q(birth_place__icontains=search_query)
            )
        else:
            # Nếu không nhập gì, hiển thị tất cả
            profiles = Profiles.objects.all()

        # 3. Trả về template (Dữ liệu ngày tháng đã là object date, hiển thị rất dễ)
        ...
        return HttpResponse(f"Lỗi hệ thống khi tìm kiếm: {str(e)}", status=500)

    # # 1. Lấy giá trị tìm kiếm từ Query String (request.GET)
    # # Lấy giá trị từ input có name="search_fields"
    # search_query = request.GET.get('search_fields', '')
...
def profile_update(request, id):
    # 1. Lấy đối tượng cần sửa từ Database
    ...
    if request.method == 'POST':
        try:
            ...
            profile_obj.birthday = request.POST.get('birthday')
            profile_obj.sex = request.POST.get('sex')
            profile_obj.birth_place = request.POST.get('birth_place')
            profile_obj.nation = request.POST.get('nation')
            profile_obj.recruitment_day = request.POST.get('recruitment_day')
            profile_obj.job_title = request.POST.get('job_title')
            profile_obj.department = request.POST.get('department')

            # 3. Lưu vào Database
            # Django sẽ tự động validate định dạng ngày tháng của DateField
            profile_obj.save()

            # Chuyển hướng về trang danh sách cập nhật
            return redirect('profiles_call_view_update')

        except Exception as e:
            # Nếu có lỗi (ví dụ: ngày tháng nhập sai định dạng)
            context = {
                'error_message': f'Cập nhật thất bại. Chi tiết: {str(e)}',
                'profile': request.POST  # Trả lại dữ liệu user vừa nhập để họ sửa
            }
            return render(request, 'notification.html', {'message': context})

    # --- Xử lý HIỂN THỊ FORM (GET) ---
    else:
        # Trả về object profile_obj đã lấy ở bước 1
        # Trong HTML, dùng {{ profile.birthday|date:'Y-m-d' }} để hiện vào ô input type="date"
        return render(request, 'profile-edit.html', {'profile': profile_obj})
...
def profiles_call_view_edit(request, id):
    try:
        # 1. Lấy duy nhất 1 đối tượng theo ID, nếu không thấy sẽ trả về lỗi 404
        profile = get_object_or_404(Profiles, id=id)

        # 2. Với Model, birthday và recruitment_day đã là kiểu date của Python.
        # Không cần dùng datetime.strptime nữa.
...
def profiles_call_view_update(request):
    try:
        # 1. Lấy tất cả dữ liệu từ Database bằng Model
        # Không cần localhost, không cần session hay cookies phức tạp
        profiles = Profiles.objects.all().order_by('-id')  # Sắp xếp mới nhất lên đầu

        # 2. Django DateField trả về object date, không cần parse strptime
        # Bạn truyền thẳng danh sách này vào template
        return render(request, 'profiles-update.html', {'profiles': profiles})

    except Exception as e:
        # Xử lý các lỗi phát sinh (ví dụ: lỗi database)
        error_message = f'Lỗi hệ thống: {str(e)}'
        return render(request, 'notification.html', {'message': error_message})
...
def profiles_list(request):
    ...
    # Phần nay tạm giữ lại để tham khảo cách dùng requests với ViewSet và Router ------------------------------------------------
    # api_url = 'http://localhost:8080/api/profile/'
    # try:
    #     # Sử dụng requests.Session để gửi kèm cookies (chứa session ID)
    #     s = requests.Session()
